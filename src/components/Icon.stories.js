...
storiesOf('Icons', module)
  ...
  .add('Icons with label', () => {
    ...
    return <div style ={{ display:'flex', alignItems: 'middle', justifyContent: 'space-around', flexFlow: 'wrap'}} >
      { arr.map((elem) => {
        return ( <div style={{padding: '0 20px', flex: '0 1 20%', alignItems: 'center', display: 'flex', marginBottom: '20px'}} key={elem[0]}><img src={elem[1]} alt={elem[0]} /><span style={{marginLeft: '10px'}}>{elem[0]}</span></div>);
      }) }
  </div>
  });