...
class FileInput extends Component {
        ...
        if(e && e.target) {
            ...
            if(file) {
                ...
                this.setState({
                    fileChosen: true,
                    fileName  : file.name,
                    error     : file.size > this.props.maxSize || file.type.length === 0 || isInvalidType
                }, () => {
                    return this.props.getValidityFunc && this.props.getValidityFunc(this.state.fileChosen && !this.state.error, file);
                })
            }
        }
    };
    ...
    get elInput() {
        ...
        if(fileTypes.length) {
            accept = fileTypes.length > 1 ? fileTypes.reduce((accumulatedTypes, nextType) => `${accumulatedTypes}, ${nextType}`) : fileTypes[0];
        }
    ...
    get elFile() {
        if(this.state.fileChosen) {
            const props = {
                ...
                children            : this.state.error ? 'TK_FILE-INPUT__UPLOAD-ERROR' : this.state.fileName,
            ...
            return <I18n className={cx('file-input__file-not-chosen')} children="TK_FILE-INPUT__FILE-NOT-CHOSEN" />
        }
    }
    ...
    get elButton() {
        ...
        return (
            <div className={cx('file-input__button', { 'file-input__button_disabled': disabled })} onClick={this.onClickOpenFileDialog}>
                <span className={cx('file-input__clip-icon')}>
                    <img src={clipIcon} />
                </span>
                <I18n className={cx('file-input__button-text')} children={buttonTitle} />
            </div>
        )
    }
    ...
    render() {
        return (
            <div className={cx('file-input__file-input-group', this.props.className)}>
                <I18n className={cx('file-input__file-input-title')} children={this.props.title} />
                {this.elFakeInput}
                {this.elInput}
                <I18n className={cx('file-input__file-input-info', { 'file-input__file-input-info_error': this.state.error })} children={this.props.info} />
            </div>
        )
    }
