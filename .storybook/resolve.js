...
module.exports = options => {
  return {
    ...
    modules: options.development
      ? ['dev_modules', 'node_modules']
      : ['node_modules'],
    unsafeCache: true,
    alias: {
      'react-dom': options.development ? '@hot-loader/react-dom' : 'react-dom',
      'server/component': path.resolve(
        options.context,
        'src',
        'server',
        'components',
      ),
      'client/component': path.resolve(
        options.context,
        'src',
        'client',
        'components',
      ...
      config: path.resolve(
        options.context,
        'configs',
        options.server ? 'config' : 'client-config',
      ),
    },
    plugins: [
